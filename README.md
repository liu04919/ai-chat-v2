...
本机开发的聊天渠道使用 Codex Proxy：先启动 Proxy 并确认账户已登录，再运行 `pnpm dev`。Worker 的 `LLM_BASE_URL` 为 `http://127.0.0.1:8080/v1`，`LLM_API_KEY` 填 Proxy 创建的本地客户端密钥，模型仍为 `gpt-5.6-sol`。修改配置后重启 Worker；`pnpm dev` 不会自动启动 Proxy。图片、Embedding 和 Rerank 继续使用各自的独立渠道。部署到其他机器时需配置该机器可访问的服务地址，不能直接沿用本机回环地址。
...
切换会话时关闭当前 SSE，但 Zustand 保留已合并内容及对应的 `lastEventId`。再次进入先刷新最新详情：同一个 Active Generation 使用 `?after=<cursor>` 续传；不同任务重新建立空投影；已无活跃任务则由数据库历史接管并清理旧投影。刷新失败保留现有内容并提示重新加载，不用旧缓存猜测任务状态。同一 EventSource 自动重连时，`Last-Event-ID` Header 优先于 URL 起点。

内容和游标在同一次 rAF 批量更新中提交，收到但尚未合并的事件不会推进缓存游标；回放按 Redis ID 去重，避免重复拼字或重复工具调用卡片。投影最多缓存最近访问的 5 个会话（包含当前会话），超限淘汰最久未访问项，内容与游标一起删除。该数量是可调整的产品上限，不是内存字节限额；不轮询、不在后台维持其他会话的 SSE。缓存被淘汰或整页刷新后，运行中任务从头回放事件，已结束任务直接读取历史。退出/换号继续清空全部缓存。
...
已接通 Agentic RAG：知识库、文档、chunk 三层结构，独立 BullMQ 入库任务，以及模型驱动的混合检索工具、精排和引用展示。知识库不依赖会话，原文件使用独立的 R2 对象。业务只保留这一条编排；传统 RAG 基线保存在 Git 提交 `7134eb0`，需要对比时使用独立 worktree，不添加模式开关或旧链路兜底。
...
左侧知识库列表中，名称右侧的红色垃圾桶支持删除整个知识库。确认后，数据库事务删除知识库、全部文档和级联分块，再逐批清理 R2 原文件；部分对象清理失败时返回警告，不假装回滚数据库，没有自动补偿。已有回答和分享引用快照保留，已删除知识库不能用于后续检索或重新生成；队列中的旧文档任务无法重新领取，已经处理中的任务也无法重新发布到已删除文档。

发送时服务端校验知识库归属，将 `knowledgeBaseId` 保存到 Generation。Worker 只在选库时注册 `search_knowledge` 工具，不预检索、不拼临时 user 资料消息。模型自行决定是否检索、改写 query 或补充检索；涉及库内事实时由指令要求先查证，问候可直接回答。工具内部复用向量＋BM25 → RRF → Rerank，账户与知识库绑定在服务端闭包，模型只能传 query（1–2000 字符）。重新生成沿用原轮选库，工具检索当时可用的文档。聊天问题仍需包含文本，但不再受旧的检索 query 2000 字符上限约束。

检索结果通过独立的 `knowledge.sources` 展示事件传到前端，保存为回答中的引用快照；仅包括编号、文档名、页码、chunk ID 和原文，不返回向量、内部评分或对象地址。多次调用按 chunk 去重、全轮稳定编号，更新同一个累计快照，不重复展示引用列表。点击编号查看片段，刷新和分享后仍可查看；删除源文件不删除已有引用。历史模型上下文保留回答文字，但成对移除旧知识库 Tool Call/Result 及引用原文，防止换库或关库后继续携带旧资料。

每轮最多三次知识库检索（并行及失败也计数），每次保留六条，最多十八条唯一来源；耗尽后从后续步骤的可用工具中移除。这个次数是保护上限，不是调参结论。`ToolLoopAgent` 管理 SDK 多步循环，最多八个模型步骤，最后一步禁止工具、留给回答。MCP/联网工具仍可并用，原始 Tool 输入/输出只在服务端保存，不发送给前端。
...
Chat 在 Worker 内处理长历史，沿用 `LLM_MODEL` 配置的 Sol / Responses 渠道；Image 不走摘要。当前预算集中在 `apps/worker/src/context/token-budget.ts`：

| 项目 | token 预算 |
| --- | ---: |
| 总输入触发压缩 | 200,000 |
| 压缩后总输入目标（触发量的 30%） | 60,000 |
| 历史摘要软目标 | 8,000 |
| 每次模型调用的输入安全预算 | 240,000 |
| 每次模型调用的输出上限（含推理） | 16,000 |
| 常规单批摘要输入目标 | 100,000 |
...
准备流程：领取 Generation 时读取当前摘要与覆盖边界之后的原始消息 → 解析工具定义（不执行工具）→ 计数 → 必要时分批摘要并原子保存 → 只为保留的附件签 URL → 进入原有工具循环。每个模型步骤再次检查输入预算，防止本轮工具结果持续增长。

摘要只覆盖较早的完整 user 轮次，至少保留最近一个历史轮次与当前问题，不拆开工具调用/结果。单个历史轮次超过常规 100k 批次目标时独占一批，但整个摘要请求仍不能超过 240k 输入安全预算。60k 是压缩后总输入目标，不是填充长度或强行截断线；若近期原文自身较大，允许高于目标，但不能超过输入安全预算。

每批摘要最多两次请求，SDK 不叠加重试：临时错误、空输出或未完整结束可重试一次；完整摘要明显超出 8k（容许 10% 偏差）时改为精简草稿一次。精简失败可以使用第一份完整草稿，不因轻微超长直接失败，但最终必须减少总输入并满足安全预算。没有有效摘要时，仅在原请求仍处于安全预算内回退；否则沿现有 Generation 失败流程退出，不丢弃当前问题。用户取消不重试、不继续回答；版本冲突不能悄悄回退。半成品从不保存。
